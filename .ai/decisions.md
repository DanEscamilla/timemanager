...
- **`libs/mailbox_kit`** — Deno email ingest pipeline (`MailboxProvider`, domain filter / wildcards, `Extractor` / `ExtractorPipeline`, approve/reject template matching via `extractSpendingCandidates`, `TemplateSpendingExtractor`, legacy heuristic `SpendingExtractor` kept for tests, `ExpenseSink` interface). Used by `mailbox-api` / `mailbox-worker`; spendmanager GraphQL types stay out of the kit (sink adapter lives in mailbox-api). Worker poll may call AI **only** to classify unmatched messages and generate approve/reject templates; extraction remains reject short-circuit + approve templates only (no heuristic).
- **`libs/ai_kit`** — Deno AI provider abstraction (`AiProvider`, `GeminiProvider`, `OpenAiCompatibleProvider`, env factory). Used by `ai-api`; use cases stay provider-agnostic so Gemini free tier can swap to a self-hosted OpenAI-compatible endpoint later.
- **`apps/ai-api`** — Internal REST AI gateway (`:3004`) for backend callers only (shared `AI_SERVICE_KEY`). Not a product GraphQL API; no DB / JWKS in v1. Use cases are hand-registered in code.
