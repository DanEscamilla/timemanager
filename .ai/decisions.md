...
- **`libs/mailbox_kit`** — Deno email ingest pipeline (`MailboxProvider`, domain filter / wildcards, `Extractor` / `ExtractorPipeline`, heuristic `SpendingExtractor`, `TemplateSpendingExtractor`, `ExpenseSink` interface). Used by `mailbox-api` / `mailbox-worker`; spendmanager GraphQL types stay out of the kit (sink adapter lives in mailbox-api).
