...
/// Registers FCM tokens with timemanager-api and bridges foreground pushes
...
class PushRegistrationService {
  ...
  Future<void> ensureInitialized() async {
    ...
    if (options == null) {
      if (kDebugMode) {
        debugPrint(
          ...
          'platform; activity reminders stay local-only',
        );
      }
      return;
    }
    ...
    try {
      await _local.ensureInitialized(kActivityNotificationConfig);
      ...
      await _push.ensureInitialized(
        config: const PushNotificationConfig(appId: 'timemanager'),
      ...
      if (!granted) {
        if (kDebugMode) {
          debugPrint(
            ...
            'activity reminders stay local-only. On web, reset site '
            'permissions for this origin if you previously clicked Block.',
          );
        }
        return;
      }
  ...
  Future<void> _showForeground(PushMessage message) async {
    ...
    final type = message.data['type'] ?? 'push';
    final key = message.data['id'] ?? message.data['activity_id'] ?? type;
    await _local.showNow(
      ImmediateNotification(
        id: _notificationId(type, key),
        title: title ?? 'Time Manager',
        body: body ?? '',
      ),
    );
  }
  ...
  static int _notificationId(String type, String key) {
    var hash = 17;
